...
scope_lb = torch.full((1,), -0.0)
scope_ub = torch.full((1,), 200.0)
...
for i in range(bo_iter):
    ...
    gp.eval()
    likelihood.eval()
    next_x, starting_candidate, ending_candidate = lcb.find_next_point(3, dimension, scope_lb, scope_ub, gp, likelihood)
    #next_x = es.find_next_point(3, dimension, scope_lb, scope_ub, gp, likelihood)
    ...
    if dimension == 1:
        ...
        visual.aquisition_plotting_1D(lcb, gp, likelihood, scope_lb, scope_ub, next_x, starting_candidate, ending_candidate)
