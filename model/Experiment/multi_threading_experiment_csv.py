...
scope_lb = torch.full((5,), -5.0)
scope_ub = torch.full((5,), 5.0)
...
def run_experiment_with_acquisition(acquisition_function, name, turn):
    ...
    for i in range(bo_iter):
        ...
        next_x, _, _ = acquisition_function.find_next_point(
            50, dimension, scope_lb, scope_ub, gp, likelihood
        )
    ...
    folder_path = os.path.join("../../results", name)
...
if __name__ == "__main__":
    for i in range(10):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = []
            futures.append(executor.submit(run_experiment_with_acquisition, lcb, "LCB", str(i)))
            futures.append(executor.submit(run_experiment_with_acquisition, ei, "EI", str(i)))
            futures.append(executor.submit(run_experiment_with_acquisition, pi, "PI", str(i)))
            futures.append(executor.submit(run_experiment_with_acquisition, es, "ES", str(i)))
            futures.append(executor.submit(run_experiment_with_acquisition, pes, "PES", str(i+1)))
            futures.append(executor.submit(run_experiment_with_acquisition, mes, "MES", str(i)))
            futures.append(executor.submit(run_experiment_with_acquisition, random, "Random", str(i)))
