...
functions = ['ackley', 'rosenbrock', 'schwefel']
methods = ['ES', 'LCB', 'EI', 'PI', 'MES', 'PES']
base_path = '/Users/rwu/dev/thesis/thesis_project_information_based_acquisition_functions'
colors = {
    'ES': 'blue',
    'LCB': 'orange',
    'EI': 'green',
...
for idx, func in enumerate(functions):
    ax = axes[idx]
    folder_prefix = f'results-{func}-20d'

    for method in methods:
        folder_path = os.path.join(base_path, folder_prefix, method)
        if not os.path.exists(folder_path):
            print(f'Warning: {folder_path} not found.')
            continue
        result = process_folder(folder_path)
        color = colors.get(method, None)
        ax.plot(result['iteration'], result['mean_observation'], label=method, color=color)
        ax.fill_between(result['iteration'], result['ci_lower'], result['ci_upper'], alpha=0.15, color=color)

    ax.set_title(f'{func.capitalize()} 20D')
...
plt.tight_layout()
plt.savefig('benchmark_comparison_20d.svg')
plt.show()
